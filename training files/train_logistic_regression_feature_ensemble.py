...
class Config:
    ...
    output_dir: Path = Path("./outputs/logistic_regression")
...
def assemble_variant_frames(
    ...
    external_df: pd.DataFrame | None,
    config: Config,
) -> tuple[pd.DataFrame | None, pd.DataFrame | None, pd.DataFrame | None, pd.DataFrame | None, pd.DataFrame | None, pd.DataFrame | None, dict[str, Any]]:
    ...
    numeric_train = pd.concat(numeric_parts_train, axis=1).apply(pd.to_numeric, errors="coerce")
    numeric_valid = pd.concat(numeric_parts_valid, axis=1).apply(pd.to_numeric, errors="coerce")
    numeric_test = pd.concat(numeric_parts_test, axis=1).apply(pd.to_numeric, errors="coerce")
    ...
    if numeric_train.empty and categorical_train.empty:
        metadata["status"] = "skipped_empty_features"
        return None, None, None, None, None, None, metadata

    metadata["num_numeric_features"] = int(numeric_train.shape[1])
    metadata["num_categorical_features"] = int(categorical_train.shape[1])
    return (
        numeric_train,
        numeric_valid,
        numeric_test,
...
def build_dense_design_matrices(
    numeric_train: pd.DataFrame,
    numeric_valid: pd.DataFrame,
    numeric_test: pd.DataFrame,
    categorical_train: pd.DataFrame,
    categorical_valid: pd.DataFrame,
    ...
    def _numeric_block(
        ...
        if train_df.shape[1] == 0:
            return (
                ...
                np.empty((len(valid_df), 0), dtype=np.float32),
                np.empty((len(test_df), 0), dtype=np.float32),
            )
        imputer = SimpleImputer(strategy="median")
        scaler = StandardScaler()
        train_arr = imputer.fit_transform(train_df.to_numpy(dtype=np.float32))
        valid_arr = imputer.transform(valid_df.to_numpy(dtype=np.float32))
        test_arr = imputer.transform(test_df.to_numpy(dtype=np.float32))
        train_scaled = scaler.fit_transform(train_arr).astype(np.float32, copy=False)
        valid_scaled = scaler.transform(valid_arr).astype(np.float32, copy=False)
        test_scaled = scaler.transform(test_arr).astype(np.float32, copy=False)
    ...
    def _categorical_block(
        ...
        train_enc = encoder.fit_transform(train_df).toarray().astype(np.float32, copy=False)
        valid_enc = encoder.transform(valid_df).toarray().astype(np.float32, copy=False)
        test_enc = encoder.transform(test_df).toarray().astype(np.float32, copy=False)
        return train_enc, valid_enc, test_enc

    train_num, valid_num, test_num = _numeric_block(numeric_train, numeric_valid, numeric_test)
    train_cat, valid_cat, test_cat = _categorical_block(
        categorical_train, categorical_valid, categorical_test
    )
    return (
        np.concatenate([train_num, train_cat], axis=1).astype(np.float32, copy=False),
        np.concatenate([valid_num, valid_cat], axis=1).astype(np.float32, copy=False),
        np.concatenate([test_num, test_cat], axis=1).astype(np.float32, copy=False),
    )


def build_sparse_design_matrices(
    numeric_train: pd.DataFrame,
    numeric_valid: pd.DataFrame,
    numeric_test: pd.DataFrame,
    categorical_train: pd.DataFrame,
    ...
    def _numeric_block(train_df: pd.DataFrame, valid_df: pd.DataFrame, test_df: pd.DataFrame):
        if train_df.shape[1] == 0:
            return (
                sparse.csr_matrix((len(train_df), 0), dtype=np.float32),
                sparse.csr_matrix((len(valid_df), 0), dtype=np.float32),
                sparse.csr_matrix((len(test_df), 0), dtype=np.float32),
            )
        imputer = SimpleImputer(strategy="median")
        scaler = StandardScaler()
        train_arr = imputer.fit_transform(train_df.to_numpy(dtype=np.float32))
        ...
        test_scaled = scaler.transform(test_arr).astype(np.float32, copy=False)
        return (
            sparse.csr_matrix(train_scaled),
            sparse.csr_matrix(valid_scaled),
            sparse.csr_matrix(test_scaled),
        )

    def _categorical_block(train_df: pd.DataFrame, valid_df: pd.DataFrame, test_df: pd.DataFrame):
        if train_df.shape[1] == 0:
            return (
                sparse.csr_matrix((len(train_df), 0), dtype=np.float32),
                sparse.csr_matrix((len(valid_df), 0), dtype=np.float32),
        ...
        valid_enc = encoder.transform(valid_df)
        test_enc = encoder.transform(test_df)
        return train_enc.tocsr(), valid_enc.tocsr(), test_enc.tocsr()
    ...
    return (
        sparse.hstack([train_num, train_cat], format="csr"),
        sparse.hstack([valid_num, valid_cat], format="csr"),
...
def get_variants() -> list[VariantSpec]:
    return [
        VariantSpec("base_numeric", ["base"], {"C": 1.0}),
        VariantSpec("base_plus_freq", ["base", "frequency_encoding"], {"C": 0.75}),
        VariantSpec("base_plus_binning", ["base", "binning"], {"C": 0.5}),
        VariantSpec("base_plus_digits", ["base", "digit_features"], {"C": 0.75}),
        VariantSpec(
            "full_feature_stack",
...
def run_variant(
    ...
    for fold, (tr_idx, va_idx) in enumerate(splits):
        ...
        if config.use_gpu:
            X_train, X_valid, X_test = build_dense_design_matrices(
                num_train, num_valid, num_test, cat_train, cat_valid, cat_test
            )
        else:
            X_train, X_valid, X_test = build_sparse_design_matrices(
                num_train, num_valid, num_test, cat_train, cat_valid, cat_test
            )
        feature_counts.append(int(X_train.shape[1]))
        log(
            f"Variant={variant.name} fold={fold + 1}/{config.n_splits} "
            f"shape_train={X_train.shape} backend={'gpu' if config.use_gpu else 'cpu'} "
            f"numeric={meta['num_numeric_features']} "
            f"categorical={meta['num_categorical_features']}"
        )

        valid_pred, test_fold_pred = fit_logistic_regression(
            X_train=X_train,
            y_train=y_train,
            X_valid=X_valid,
            X_test=X_test,
        ...
        del X_train, X_valid, X_test, num_train, num_valid, num_test, cat_train, cat_valid, cat_test
        gc.collect()

    cv_auc = float(roc_auc_score(y, oof))
    test_pred = (test_pred_sum / config.n_splits).astype(np.float32)
    np.save(config.output_dir / f"oof_logreg_{variant.name}.npy", oof.astype(np.float32))
    np.save(config.output_dir / f"test_logreg_{variant.name}.npy", test_pred.astype(np.float32))
...
def main() -> None:
    ...
    for variant in variants:
        ...
        try:
            ...
            if row["status"] == "ok":
                oof_manifest[variant.name] = np.load(
                    config.output_dir / f"oof_logreg_{variant.name}.npy"
                )
                test_manifest[variant.name] = np.load(
                    config.output_dir / f"test_logreg_{variant.name}.npy"
                )
    ...
    summary_df.to_csv(config.output_dir / "summary_logreg.csv", index=False)
    save_manifest(
        oof_manifest.reset_index(drop=True),
        config.output_dir / "all_oof_logreg.parquet",
        config.save_parquet,
    )
    save_manifest(
        test_manifest.reset_index(drop=True),
        config.output_dir / "all_test_logreg.parquet",
        config.save_parquet,
    )
    log("Logistic regression feature ensemble run complete.")
