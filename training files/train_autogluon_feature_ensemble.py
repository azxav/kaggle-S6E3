...
try:
    ...
    raise ImportError(
        "train_autogluon_feature_ensemble.py requires autogluon.tabular. "
        "Install it in the notebook environment, for example: pip install autogluon.tabular"
    ) from exc
...
class Config:
    ...
    output_dir: Path = Path("./outputs/autogluon")
    ...
    time_limit_per_fold: int | None = None
    use_xgb: bool = True
    use_cat: bool = True
    use_nn_torch: bool = True
    use_fastai: bool = False
    selected_variants: list[str] = ["ag_gp_mix", "ag_hybrid_full", "ag_hybrid_mix"]
...
def make_base_frames(
    ...
    for col in categorical_cols:
        out_train[col] = normalize_categorical(train_df[col]).astype("category")
        out_valid[col] = normalize_categorical(valid_df[col]).astype("category")
        out_test[col] = normalize_categorical(test_df[col]).astype("category")
...
def build_binning_features(
    ...
    for col in numeric_cols:
        ...
        if len(q_edges) >= 3:
            out_train[f"{col}__qbin"] = pd.cut(train_num, q_edges, include_lowest=True, duplicates="drop").astype(str).replace({"nan": "__MISSING__"}).astype("category")
            out_valid[f"{col}__qbin"] = pd.cut(valid_num, q_edges, include_lowest=True, duplicates="drop").astype(str).replace({"nan": "__MISSING__"}).astype("category")
            out_test[f"{col}__qbin"] = pd.cut(test_num, q_edges, include_lowest=True, duplicates="drop").astype(str).replace({"nan": "__MISSING__"}).astype("category")
        ...
        if np.isfinite(low) and np.isfinite(high) and high > low:
            ...
            if len(cut_edges) >= 3:
                out_train[f"{col}__wbin"] = pd.cut(train_num, cut_edges, include_lowest=True, duplicates="drop").astype(str).replace({"nan": "__MISSING__"}).astype("category")
                out_valid[f"{col}__wbin"] = pd.cut(valid_num, cut_edges, include_lowest=True, duplicates="drop").astype(str).replace({"nan": "__MISSING__"}).astype("category")
                out_test[f"{col}__wbin"] = pd.cut(test_num, cut_edges, include_lowest=True, duplicates="drop").astype(str).replace({"nan": "__MISSING__"}).astype("category")
        for divisor in config.rounded_bin_divisors:
            out_train[f"{col}__round_{divisor}"] = np.floor(train_num / divisor).fillna(-9999).astype(np.int32).astype(str).astype("category")
            out_valid[f"{col}__round_{divisor}"] = np.floor(valid_num / divisor).fillna(-9999).astype(np.int32).astype(str).astype("category")
            out_test[f"{col}__round_{divisor}"] = np.floor(test_num / divisor).fillna(-9999).astype(np.int32).astype(str).astype("category")
    return out_train, out_valid, out_test
...
def assemble_variant_features(
    ...
    metadata: dict[str, Any] = {"status": "ok", "feature_families": list(variant.feature_families)}

    if "all_as_categorical" in variant.feature_families:
        final_train = pd.concat(
            [base_train[numeric_cols].round(3).astype(str).astype("category"), base_train[categorical_cols]],
            axis=1,
        ).add_prefix("allcat__")
        final_valid = pd.concat(
            [base_valid[numeric_cols].round(3).astype(str).astype("category"), base_valid[categorical_cols]],
            axis=1,
        ).add_prefix("allcat__")
        final_test = pd.concat(
            [base_test[numeric_cols].round(3).astype(str).astype("category"), base_test[categorical_cols]],
            axis=1,
        ).add_prefix("allcat__")
    else:
        final_train = pd.concat([base_train[numeric_cols], base_train[categorical_cols]], axis=1)
        final_valid = pd.concat([base_valid[numeric_cols], base_valid[categorical_cols]], axis=1)
        final_test = pd.concat([base_test[numeric_cols], base_test[categorical_cols]], axis=1)
...
def get_hyperparameters(config: Config, recipe: str) -> dict[str, Any]:
    params: dict[str, Any] = {}
    ...
    if recipe == "gbm_cat":
        ...
        if config.use_xgb:
            params["XGB"] = {}
    elif recipe == "cat_only":
        if not config.use_cat:
            raise ValueError("CAT recipe requested but USE_CAT is False")
        params["CAT"] = {}
    elif recipe == "nn_only":
        if config.use_nn_torch:
            params["NN_TORCH"] = {}
        if config.use_fastai:
            params["FASTAI"] = {}
        if not params:
            raise ValueError("NN recipe requested but USE_NN_TORCH and USE_FASTAI are both False")
    elif recipe == "mix":
        params["GBM"] = {}
        if config.use_cat:
            params["CAT"] = {}
...
def predict_positive_proba(predictor: TabularPredictor, X: pd.DataFrame) -> np.ndarray:
    proba = predictor.predict_proba(X)
    if isinstance(proba, pd.Series):
        return proba.to_numpy(dtype=np.float32)
...
def get_variants() -> list[VariantSpec]:
    return [
        VariantSpec("ag_base_gbm_cat", ["base"], "gbm_cat"),
        VariantSpec("ag_base_freq_gbm_cat", ["base", "frequency_encoding"], "gbm_cat"),
        VariantSpec("ag_base_binning_gbm_cat", ["base", "binning"], "gbm_cat"),
        VariantSpec("ag_allcat_cat_only", ["all_as_categorical"], "cat_only"),
        VariantSpec("ag_base_nn_only", ["base"], "nn_only"),
        VariantSpec("ag_base_digits_nn", ["base", "digit_features"], "nn_only"),
        VariantSpec("ag_base_external_stats_mix", ["base", "external_stats"], "mix"),
        VariantSpec("ag_hybrid_mix", ["base", "binning", "frequency_encoding"], "mix"),
        VariantSpec("ag_hybrid_full", ["base", "binning", "digit_features", "frequency_encoding", "external_stats"], "mix"),
        VariantSpec("ag_gp_mix", ["base", "gp_features"], "mix"),
    ]
...
def run_variant(
    ...
    for fold, (tr_idx, va_idx) in enumerate(splits):
        ...
        train_fold_df = X_train.copy()
        valid_fold_df = X_valid.copy()
        test_fold_df = X_test.copy()
        train_fold_df[config.target_col] = y_train.astype(np.int8)
        valid_fold_df[config.target_col] = y_valid.astype(np.int8)

        model_dir = Path(tempfile.mkdtemp(prefix=f"ag_{variant.name}_fold{fold}_", dir=config.output_dir))
        ...
        try:
            ...
            predictor.fit(
                train_data=train_fold_df,
                tuning_data=valid_fold_df,
                hyperparameters=get_hyperparameters(config, variant.recipe),
                presets=config.presets,
                time_limit=config.time_limit_per_fold,
                num_bag_folds=0,
                num_stack_levels=0,
                calibrate_decision_threshold=False,
                verbosity=1,
            )
            valid_pred = predict_positive_proba(predictor, X_valid)
            test_fold_pred = predict_positive_proba(predictor, X_test)
        finally:
            del train_fold_df, valid_fold_df, test_fold_df
            ...
            try:
                ...
                log(f"Warning: failed to cleanup {model_dir}: {exc}")
    ...
    np.save(config.output_dir / f"oof_ag_{variant.name}.npy", oof.astype(np.float32))
    np.save(config.output_dir / f"test_ag_{variant.name}.npy", test_pred.astype(np.float32))
...
def main() -> None:
    ...
    for variant in variants:
        ...
        try:
            ...
            if row["status"] == "ok":
                oof_manifest[variant.name] = np.load(config.output_dir / f"oof_ag_{variant.name}.npy")
                test_manifest[variant.name] = np.load(config.output_dir / f"test_ag_{variant.name}.npy")
            log(
    ...
    summary_df.to_csv(config.output_dir / "summary_autogluon.csv", index=False)
    save_manifest(oof_manifest.reset_index(drop=True), config.output_dir / "all_oof_autogluon.parquet", config.save_parquet)
    save_manifest(test_manifest.reset_index(drop=True), config.output_dir / "all_test_autogluon.parquet", config.save_parquet)
    log("AutoGluon feature ensemble run complete.")
