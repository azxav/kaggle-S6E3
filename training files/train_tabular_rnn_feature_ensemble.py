...
class Config:
    ...
    output_dir: Path = Path("/kaggle/working/outputs/tabular_rnn")
...
def load_model_class():
    try:
        ...
        return TabulaRNNClassifier
    except Exception:
        pass
    try:
        from deeptab.models import TabulaRNNClassifier  # type: ignore
        return TabulaRNNClassifier
    except Exception as exc:
        raise ImportError(
            "train_tabular_rnn_feature_ensemble.py requires mambular. "
            "Install it in the Kaggle notebook, for example: pip install mambular"
        ) from exc
...
def prepare_model_frames(
    ...
    numeric_cols = [col for col in train_out.columns if pd.api.types.is_numeric_dtype(train_out[col])]
    categorical_cols = [col for col in train_out.columns if col not in numeric_cols]
    for col in numeric_cols:
        tr = pd.to_numeric(train_out[col], errors="coerce").astype(np.float32)
        va = pd.to_numeric(valid_out[col], errors="coerce").astype(np.float32)
        te = pd.to_numeric(test_out[col], errors="coerce").astype(np.float32)
        fill_value = float(tr.median()) if tr.notna().any() else 0.0
        train_out[col] = tr.fillna(fill_value).astype(np.float32)
        valid_out[col] = va.fillna(fill_value).astype(np.float32)
        test_out[col] = te.fillna(fill_value).astype(np.float32)
    for col in categorical_cols:
        train_out[col] = normalize_categorical(train_out[col])
        valid_out[col] = normalize_categorical(valid_out[col])
        test_out[col] = normalize_categorical(test_out[col])
...
def fit_model(
    ...
    try:
        ...
        model = ModelClass()
    # Pass the CV fold's validation set so early stopping uses it instead of
    # carving off a random slice from X_train.
    fit_kwargs: dict = {}
...
def run_variant(
    ...
    for fold, (tr_idx, va_idx) in enumerate(splits):
        ...
        X_train, X_valid, X_test = prepare_model_frames(X_train, X_valid, X_test)
        ...
        model = fit_model(X_train, y_train, config, X_val=X_valid, y_val=y_valid)
        ...
        if config.run_mode == "submission":
            test_fold_pred = predict_positive_proba(model, X_test)
    ...
    if config.run_mode == "submission":
        ...
        np.save(out_dir / f"oof_tabular_rnn_{variant.name}.npy", oof.astype(np.float32))
        np.save(out_dir / f"test_tabular_rnn_{variant.name}.npy", test_pred)
    return row
...
def main() -> None:
    ...
    for variant in variants:
        ...
        try:
            ...
            if config.run_mode == "submission" and row["status"] == "ok":
                oof_manifest[variant.name] = np.load(out_dir / f"oof_tabular_rnn_{variant.name}.npy")
                test_manifest[variant.name] = np.load(out_dir / f"test_tabular_rnn_{variant.name}.npy")
    ...
    summary_df.to_csv(out_dir / f"summary_tabular_rnn_{config.run_mode}.csv", index=False)
    if config.run_mode == "submission":
        save_manifest(
            oof_manifest.reset_index(drop=True),
            out_dir / "all_oof_tabular_rnn_submission.parquet",
            config.save_parquet,
        )
        save_manifest(
            test_manifest.reset_index(drop=True),
            out_dir / "all_test_tabular_rnn_submission.parquet",
            config.save_parquet,
        )
    log("Tabular RNN feature ensemble run complete.")
