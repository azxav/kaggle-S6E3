...
class Config:
    ...
    output_dir: Path = Path("./outputs/lightgbm")
...
def align_categorical_categories(
    train_df: pd.DataFrame,
    valid_df: pd.DataFrame,
    test_df: pd.DataFrame,
    categorical_cols: list[str],
    ...
    aligned_valid = valid_df.copy()
    aligned_test = test_df.copy()
    for col in categorical_cols:
        train_series = normalize_categorical(aligned_train[col])
        valid_series = normalize_categorical(aligned_valid[col])
        test_series = normalize_categorical(aligned_test[col])
        categories = pd.Index(
            pd.concat([train_series, valid_series, test_series], axis=0, ignore_index=True).unique()
        )
        aligned_train[col] = pd.Categorical(train_series, categories=categories)
        aligned_valid[col] = pd.Categorical(valid_series, categories=categories)
        aligned_test[col] = pd.Categorical(test_series, categories=categories)
    return aligned_train, aligned_valid, aligned_test
...
def assemble_variant_features(
    ...
    final_train, final_valid, final_test = align_categorical_categories(
        final_train, final_valid, final_test, cat_cols
    )
    metadata["categorical_cols"] = cat_cols
...
def fit_lightgbm(
    ...
    try:
        ...
        raise ImportError(
            "train_lightgbm_feature_ensemble.py requires lightgbm. "
            "Install it in the notebook environment, for example: pip install lightgbm"
        ) from exc
    ...
    fit_kwargs: dict[str, Any] = {
        ...
        "eval_metric": "auc",
        "callbacks": [early_stopping(config.early_stopping_rounds), log_evaluation(0)],
    }
    if categorical_cols:
...
def run_variant(
    ...
    for fold, (tr_idx, va_idx) in enumerate(splits):
        ...
        valid_pred, test_fold_pred = fit_lightgbm(
            X_train=X_train_df,
            y_train=y_train,
            X_valid=X_valid_df,
            y_valid=y_valid,
    ...
    np.save(config.output_dir / f"oof_lgb_{variant.name}.npy", oof.astype(np.float32))
    np.save(config.output_dir / f"test_lgb_{variant.name}.npy", test_pred.astype(np.float32))
...
def main() -> None:
    ...
    for variant in variants:
        ...
        try:
            ...
            if row["status"] == "ok":
                oof_manifest[variant.name] = np.load(config.output_dir / f"oof_lgb_{variant.name}.npy")
                test_manifest[variant.name] = np.load(config.output_dir / f"test_lgb_{variant.name}.npy")
    ...
    summary_df.to_csv(config.output_dir / "summary_lightgbm.csv", index=False)
    save_manifest(
        oof_manifest.reset_index(drop=True),
        config.output_dir / "all_oof_lightgbm.parquet",
        config.save_parquet,
    )
    save_manifest(
        test_manifest.reset_index(drop=True),
        config.output_dir / "all_test_lightgbm.parquet",
        config.save_parquet,
    )
    log("LightGBM feature ensemble run complete.")
