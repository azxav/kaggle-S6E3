...
class Config:
    ...
    output_dir: Path = Path("/kaggle/working/outputs/excelformer")
...
def import_torch_frame() -> dict[str, Any]:
    try:
        ...
        raise ImportError(
            "train_excelformer_feature_ensemble.py requires torch and pytorch-frame. "
...
def build_runtime_config(config: Config) -> RuntimeConfig:
...
def build_excelformer_inputs(
    ...
    for col in train_dataset.tensor_frame.col_names_dict.get(stype.numerical, []):
        transformed_stats.setdefault(col, train_dataset.col_stats[col])

    sorter = MutualInformationSort(task_type=TaskType.BINARY_CLASSIFICATION)
    sorter.fit(train_tf, transformed_stats)
    train_tf = sorter(train_tf)
    valid_tf = sorter(valid_tf)
    test_tf = sorter(test_tf)
    return train_tf, valid_tf, test_tf, dict(sorter.transformed_stats)


def build_model(train_tf, transformed_stats: dict[str, Any], runtime_config: RuntimeConfig):
    deps = import_torch_frame()
    stype = deps["torch_frame"].stype
    ...
    return ExcelFormer(
        ...
        num_heads=runtime_config.num_heads,
...
def make_loader(dataset_or_tf, batch_size: int, shuffle: bool, num_workers: int):
...
def extract_targets(dataset_or_tf) -> np.ndarray:
    if hasattr(dataset_or_tf, "y") and dataset_or_tf.y is not None:
        return dataset_or_tf.y.detach().cpu().numpy().astype(np.float32)
    if hasattr(dataset_or_tf, "tensor_frame") and hasattr(dataset_or_tf.tensor_frame, "y"):
        return dataset_or_tf.tensor_frame.y.detach().cpu().numpy().astype(np.float32)
    raise ValueError("Validation targets were not found on the torch-frame object")


def predict_proba(model, dataset_or_tf, batch_size: int, device, num_workers: int) -> np.ndarray:
    deps = import_torch_frame()
    torch = deps["torch"]
    loader = make_loader(dataset_or_tf, batch_size=batch_size, shuffle=False, num_workers=num_workers)
...
def train_one_fold(
    ...
    deps = import_torch_frame()
    torch = deps["torch"]
    model = build_model(train_tf, transformed_stats, runtime_config).to(device)
    ...
    y_valid = extract_targets(valid_tf)
...
def run_variant(
    ...
    for fold, (tr_idx, va_idx) in enumerate(splits):
        ...
        train_tf, valid_tf, test_tf, transformed_stats = build_excelformer_inputs(
            X_train, y_train, X_valid, y_valid, X_test
        )
        model, valid_pred, test_fold_pred = train_one_fold(
            train_tf, valid_tf, test_tf, transformed_stats, runtime_config, device
        )
        ...
        del X_train, X_valid, X_test, train_tf, valid_tf, test_tf, transformed_stats, model
        gc.collect()
    ...
    if config.run_mode == "submission":
        ...
        np.save(out_dir / f"oof_excelformer_{variant.name}.npy", oof.astype(np.float32))
        np.save(out_dir / f"test_excelformer_{variant.name}.npy", test_pred)
    return row
...
def main() -> None:
    ...
    for variant in variants:
        ...
        try:
            ...
            if config.run_mode == "submission" and row["status"] == "ok":
                oof_manifest[variant.name] = np.load(out_dir / f"oof_excelformer_{variant.name}.npy")
                test_manifest[variant.name] = np.load(out_dir / f"test_excelformer_{variant.name}.npy")
    ...
    summary_df.to_csv(out_dir / f"summary_excelformer_{config.run_mode}.csv", index=False)
    if config.run_mode == "submission":
        save_manifest(
            oof_manifest.reset_index(drop=True),
            out_dir / "all_oof_excelformer_submission.parquet",
            config.save_parquet,
        )
        save_manifest(
            test_manifest.reset_index(drop=True),
            out_dir / "all_test_excelformer_submission.parquet",
            config.save_parquet,
        )
    log("ExcelFormer feature ensemble run complete.")
